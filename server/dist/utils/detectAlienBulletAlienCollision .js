...
exports.detectAlienBulletAlienCollision = void 0;
const detectAlienBulletAlienCollision = (room) => {
    room.alienBullets = room.alienBullets.filter((bullet) => {
        let hit = false;
        for (let i = 0; i < room.aliens.length; i++) {
            const alien = room.aliens[i];
            const collided = bullet.x < alien.x + alien.width &&
...
exports.detectAlienBulletAlienCollision = detectAlienBulletAlienCollision;
