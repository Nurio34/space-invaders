...
exports.detectAlienAlienCollision = void 0;
const detectAlienAlienCollision = (room) => {
    const aliens = room.aliens;
    for (let i = 0; i < aliens.length; i++) {
        const alienA = aliens[i];
        for (let j = i + 1; j < aliens.length; j++) {
            const alienB = aliens[j];
            const isColliding = alienA.x < alienB.x + alienB.width &&
...
exports.detectAlienAlienCollision = detectAlienAlienCollision;
