...
exports.detectBulletPlayerCollision = void 0;
const detectBulletPlayerCollision = (room) => {
    room.bullets = room.bullets.filter((bullet) => {
        if (!bullet.id)
            return true; // skip invalid bullet
        let hit = false;
        for (const playerId in room.players) {
            const player = room.players[playerId];
            if (player.isPlayerDead())
                continue;
            const collision = bullet.x < player.x + player.size &&
...
exports.detectBulletPlayerCollision = detectBulletPlayerCollision;
